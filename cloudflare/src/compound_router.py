...
def _split(text):
    ...
    out=[]
    # Conectores fortes que normalmente introduzem uma nova intenção/domínio.
...
async def _academic_piece(db,uid,segment):
    ...
    # Consulta de aula em um dia: confirma usando a grade real, sem inferir presença.
    if subject and target_date and any(x in n for x in ("tenho","tem aula","aula","certo","ne","né")):
    ...
    # Pensar/cogitar faltar é comentário, nunca grava ausência.
    ...
    # Pode coexistir com a intenção de faltar na mesma cláusula.
...
async def handle_message(db,token,message):
    ...
    # Também entra quando uma só cláusula acadêmica contém consulta + ação/comentário.
    ...
    # Não sequestra mensagem composta se não conseguiu resolver pelo menos dois blocos.
