...
def clean_title(text,kind):
    s=remove_temporal(strip_butler(text))
    patterns=[r"^(?:eu\s+)?tenho\s+",r"^(?:eu\s+)?vou\s+(?:ter|ao|à|a|no|na)\s+",r"^(?:marca|marcar|anota|anotar)\s+(?:um\s+)?(?:compromisso\s+)?(?:de|para|com)?\s*"] if kind=="compromisso" else [r"^(?:me\s+)?lembra(?:-me)?\s+de\s+",r"^(?:eu\s+)?preciso\s+(?:de\s+)?",r"^(?:eu\s+)?tenho\s+que\s+",r"^(?:anota|anotar|adiciona|adicionar)\s+(?:uma\s+)?tarefa\s+(?:de|para)?\s*"]
...
def interpret(text,today=None):
    ...
    if re.match(r"^(?:ta faltando|esta faltando|falta|faltam)\b",n) or any(x in n for x in ("na lista de mercado","na lista da feira","na lista de compras")):
        ...
        return ("grocery_add",{"items":[x.strip() for x in re.split(r",|\s+e\s+",cleaned) if x.strip()]})
    m=re.match(r"^(?:eu\s+)?(?:preciso|tenho que)\s+comprar\s+(.+)$",n)
    if m:
        parts=[x.strip() for x in re.split(r",|\s+e\s+",remove_temporal(m.group(1))) if x.strip()]
        known={normalize(x) for x in GROCERY_TERMS}
    ...
    if any(x in n for x in ("tenho consulta","tenho dentista","tenho medico","tenho reuniao","tenho prova","vou ao medico","vou no dentista","vou ter reuniao","marca compromisso","anota compromisso")) or (d and t and any(x in n for x in ("dentista","consulta","medico","reuniao","prova","entrevista"))):
        return ("appointment_create",{"title":clean_title(raw,"compromisso"),"date":d,"time":t})
    if any(x in n for x in ("me lembra de","lembra-me de","preciso ","tenho que ","anota uma tarefa","adiciona uma tarefa")):
        return ("task_create",{"title":clean_title(raw,"tarefa"),"date":d,"time":t,"reminder_request":"lembra" in n})
    return None
