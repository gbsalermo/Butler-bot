...
A Etapa 1.2 centraliza aqui a persistência de lembretes naturais. A identificação
do ato linguístico vem de ``language_primitives``; este módulo continua dono do
wizard de data/hora e da criação no D1.
"""
import re
...
def _looks_like_request(text):
    """Reconhece pedido positivo de lembrete pela família linguística comum.

    ``não me lembra...`` é uma negação da ação e não cria nada. Já a construção
    idiomática ``não deixa eu esquecer...`` continua sendo um pedido positivo.
    """
    return language.is_positive_action_request(text, "reminder")


def _clean_title(text):
    value = text.strip()
    prefixes = (
        r"^(?:Butler[,!:\-]?\s*)?(?:por\s+favor\s+)?(?:só\s+|so\s+)?(?:me\s+)?(?:lembra|lembre|avisa|avise|recorda|recorde)(?:-me)?\s*(?:de\s+|que\s+|pra\s+|para\s+)?",
        r"^(?:Butler[,!:\-]?\s*)?(?:por\s+favor\s+)?lembra\s+eu\s*(?:de\s+|que\s+|pra\s+|para\s+)?",
        r"^(?:Butler[,!:\-]?\s*)?(?:cria|crie|criar|faz|faça|faca|fazer|anota|anote|anotar|coloca|coloque|colocar|adiciona|adicione|adicionar|bota|botar|marca|marque|marcar)\s+(?:aí\s+|ai\s+)?(?:um\s+|uma\s+)?(?:lembrete|aviso)\s*(?:de\s+|para\s+|pra\s+)?",
        ...
        r"^(?:Butler[,!:\-]?\s*)?(?:me\s+)?chama\s+(?:a\s+)?atenção\s*(?:pra|para|de|que)?\s*",
        r"^(?:Butler[,!:\-]?\s*)?(?:quando\s+der|quando\s+chegar)\b.*?\b(?:me\s+avisa|me\s+lembra)\s*(?:de\s+|que\s+|pra\s+|para\s+)?",
    )
