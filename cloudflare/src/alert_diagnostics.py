...
async def _notified(db, uid, key):
    row = await db.prepare("SELECT id FROM notification_log WHERE user_id=? AND notification_key=?").bind(uid, key).first()
    return bool(row)
...
async def handle_message(db, token, message):
    ...
    for item in items:
        iid = int(_row(item, "id")); kind = _row(item, "kind"); details = _row(item, "details") or ""; due = _row(item, "due_time")
        if details.startswith("exam:"):
            continue
        simple = details == "simple_reminder"
        advance = 5 if (kind == "compromisso" and not simple) else 0
        try:
            h,m = map(int,due.split(":")); desired = now.replace(hour=h,minute=m,second=0,microsecond=0)-timedelta(minutes=advance)
        except Exception:
            continue
    ...
    for routine in routines:
        if not routine_integration._applies(_row(routine,"weekdays"), today):
            continue
        rid = int(_row(routine,"id")); scheduled = routine_integration._times(_row(routine,"time_hhmm")); done = await routine_integration._status(db,rid,today,scheduled)
        ...
        for target in scheduled:
            ...
            if target in done:
                ...
                try:
                    h,m=map(int,target.split(":")); desired=now.replace(hour=h,minute=m,second=0,microsecond=0)
