"""Ajustes de qualidade para rotinas e frases informais de mercado.

Chamado pelo dispatcher de produção. Este módulo não define mais política temporal
de tarefas/compromissos: ``reliable_reminders.py`` é a autoridade única desses
avisos. O único monkeypatch restante aqui é a decisão de checkpoint de rotina.
"""
...
async def _save_checkpoint_smart(db, uid, routine, target_time=None):
    """Marca um checkpoint sem exigir resposta exatamente no minuto do lembrete."""
    ...
    await db.prepare(
        "INSERT INTO routine_logs(routine_id,log_date,status) VALUES(?,?,?) "
        "ON CONFLICT(routine_id,log_date) DO UPDATE SET status=excluded.status"
    ).bind(routine_id, today.isoformat(), status).run()

    if complete:
        category = _row(routine, "category")
        goal = await db.prepare(
            "SELECT id FROM goals WHERE user_id=? AND lower(name)=lower(?) LIMIT 1"
        ).bind(uid, category).first()
        if goal:
            note = f"rotina:{routine_id}"
            gid = int(_row(goal, "id"))
            await db.prepare(
                "INSERT INTO goal_progress(goal_id,amount,log_date,note) "
                "SELECT ?,1,?,? WHERE NOT EXISTS("
                "SELECT 1 FROM goal_progress WHERE goal_id=? AND log_date=? AND note=?)"
            ).bind(gid, today.isoformat(), note, gid, today.isoformat(), note).run()
        ...
        if match:
            ...
            if items:
                ...
                await send_message(
                    token,
                    int(chat_id),
                    "🛒 Anotado: " + ", ".join(items) + ". Acabou em casa, virou problema da lista. 😌",
                    reply_markup=_kb(GROCERY_KB),
                )
                return True

    # Confirmação natural de rotina fora do lembrete.
    if re.match(r"^(?:ja\s+)?(?:bebi|fiz|cumpri|completei|terminei)\b", n):
        ...
        if len(matches) == 1:
            ...
            if complete:
                ...
                msg = (
                    f"✅ {_row(routine,'name')}: {len(done)}/{len(scheduled)} checkpoint(s). "
                    "Pode confirmar depois do horário também; eu não exijo pontualidade de relógio suíço."
                )
...
def install():
    """Instala somente a política inteligente de checkpoints de rotina."""
