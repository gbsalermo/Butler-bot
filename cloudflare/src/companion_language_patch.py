...
async def handle_message(db, token, message):
    ...
    n = v2._norm(text)
    if "prova" not in n or not ("duas" in n or "2" in n):
        return False
    if "mesmo dia" not in n and "nao sei o que fazer" not in n:
        return False

    m = re.search(r"daqui a (uma|um|duas|dois|tres|quatro) semanas?", n)
    if not m:
        return False
    weeks = WORD_NUMBERS.get(m.group(1))
    if not weeks:
        return False
