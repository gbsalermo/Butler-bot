...
def task_context(item_id: int) -> dict:
    ...
    if row["due_date"]:
        try:
            due = datetime.fromisoformat(f"{row['due_date']}T{row['due_time'] or '23:59'}")
...
def task_done_comment(item_id: int) -> str:
    ...
    if n == 3:
        return "🏳️ Finalmente. Três adiamentos depois, a responsabilidade perdeu a guerra de desgaste."
    if n == 2:
        return "😮‍💨 Feito. Precisou de duas prorrogações, mas eu vou contar como vitória."
    if ctx["overdue"]:
        return "😏 Feito. Demorou mais do que deveria, mas chegamos lá. Não vou estragar o momento."
...
def load_evolution_comment(exercise_name: str) -> str | None:
    ...
    return f"⚖️ Carga estável entre {first:g} kg e {last:g} kg desde a semana {first_week}. Nem todo progresso precisa parecer uma escada todo treino."
