...
def _parse_exercise(line: str) -> tuple[str, int | None, str | None, str | None] | None:
    ...
    candidate = " ".join(parts) if len(parts) >= 2 else raw
    ...
    name = before or (parts[0] if parts else "")
    if len(name) < 2:
        return None
    load = None
    load_match = re.search(r"((?:\d+(?:[.,]\d+)?\s*(?:kg|kgs|quilos?)(?:\s+cada\s+lado)?)|peso\s+corporal|sem\s+carga|el[aá]stico|halteres?|barra)", after, flags=re.I)
    if load_match:
...
def parse_workout_text(text: str) -> list[ImportedWorkoutExercise]:
    ...
    for line in lines:
        ...
        if day:
            current_day, current_focus, waiting_focus = day, focus, focus is None
            continue
        ...
        if parsed:
            ...
            exercises.append(ImportedWorkoutExercise(current_day, current_focus or "Treino", name, sets, reps, load))
            waiting_focus = False
            continue
