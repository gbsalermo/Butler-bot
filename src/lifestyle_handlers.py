...
def _kind_from_text(text: str) -> str | None:
    ...
    if "compromisso" in t:
        return "compromisso"
    if "pendência" in t or "pendencia" in t:
        return "pendencia"
    return None
...
def _parse_time(value: str):
    v = value.strip().lower()
    if v in {"sem horário", "sem horario", "-", "nenhum"}:
        return None
    try:
...
async def add_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Qual é o título?\n\nVocê pode cancelar a qualquer momento pelo botão abaixo.",
...
async def add_title(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(text) < 2:
        await update.message.reply_text("Informe um título válido.", reply_markup=CANCEL_KEYBOARD)
        return ITEM_TITLE
    context.user_data["new_daily_item"]["title"] = text
    await update.message.reply_text(
        "Para qual data? `DD/MM/AAAA`, `hoje`, `amanhã` ou `sem data`.",
        parse_mode="Markdown",
        reply_markup=CANCEL_KEYBOARD,
    )
    return ITEM_DATE
...
async def add_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if value is False:
        await update.message.reply_text("Data inválida.", reply_markup=CANCEL_KEYBOARD)
        return ITEM_DATE
    context.user_data["new_daily_item"]["due_date"] = value
    await update.message.reply_text(
        "Qual o horário? `HH:MM` ou `sem horário`.",
        parse_mode="Markdown",
        reply_markup=CANCEL_KEYBOARD,
    )
    return ITEM_TIME
...
async def add_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if value is False:
        await update.message.reply_text("Horário inválido.", reply_markup=CANCEL_KEYBOARD)
        return ITEM_TIME
    context.user_data["new_daily_item"]["due_time"] = value
    if value:
        await update.message.reply_text(
            "Quanto tempo antes devo avisar? Ex.: `5`, `10`, `30` minutos ou `0` para avisar na hora.",
            parse_mode="Markdown",
            reply_markup=CANCEL_KEYBOARD,
        )
        return ITEM_REMINDER
    context.user_data["new_daily_item"]["reminder_minutes"] = 10
    await update.message.reply_text("Alguma observação? `-` se não houver.", parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
    return ITEM_DETAILS
...
async def add_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not value.isdigit() or int(value) > 1440:
        await update.message.reply_text("Informe minutos entre 0 e 1440.", reply_markup=CANCEL_KEYBOARD)
        return ITEM_REMINDER
...
async def list_kind(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not rows:
        await update.message.reply_text("Nada pendente por aqui.", reply_markup=_menu(kind))
        return
    parts = [f"{LABELS[kind][0]} *{LABELS[kind][2]} pendentes*\n"]
    for r in rows:
        when = ""
        if r["due_date"]:
            when += " — " + datetime.fromisoformat(r["due_date"]).strftime("%d/%m/%Y")
        if r["due_time"]:
            when += f" às {r['due_time']}"
        parts.append(f"`#{r['id']}` *{r['title']}*{when}")
        if r["details"]:
            parts.append(f"   {r['details']}")
        if r["due_time"]:
            parts.append(f"   🔔 {r['reminder_minutes']} min antes")
...
async def select_start(update: Update, context: ContextTypes.DEFAULT_TYPE, action: str) -> int:
    ...
    rows = list_items(kind=kind)
    if not rows:
        await update.message.reply_text("Não há itens pendentes.", reply_markup=_menu(kind))
        return ConversationHandler.END
    context.user_data["daily_action"] = {"kind": kind, "action": action}
    listing = "\n".join(f"#{r['id']} — {r['title']}" for r in rows)
    await update.message.reply_text(
        f"Qual item? Digite o número.\n\n{listing}",
        reply_markup=CANCEL_KEYBOARD,
    )
...
async def complete_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await select_start(update, context, "complete")


async def edit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await select_start(update, context, "edit")


async def delete_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await select_start(update, context, "delete")
...
async def complete_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not value.isdigit() or not complete_item(int(value)):
        await update.message.reply_text("Não encontrei esse item.", reply_markup=_menu(data["kind"]))
        return ConversationHandler.END
...
async def edit_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not value.isdigit() or not get_item(int(value)):
        await update.message.reply_text("Número inválido.", reply_markup=CANCEL_KEYBOARD)
        return EDIT_ID
    context.user_data["daily_action"]["id"] = int(value)
    kb = ReplyKeyboardMarkup(
        [["Título", "Data"], ["Horário", "Lembrete"], ["Observação"], ["❌ Cancelar ação"]],
...
async def edit_field(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    allowed = {
        "título": "title", "titulo": "title", "data": "due_date",
        "horário": "due_time", "horario": "due_time", "lembrete": "reminder_minutes",
        "observação": "details", "observacao": "details",
    }
    if field not in allowed:
        await update.message.reply_text("Escolha uma das opções.")
        return EDIT_FIELD
    context.user_data["daily_action"]["field"] = allowed[field]
    await update.message.reply_text("Informe o novo valor:", reply_markup=CANCEL_KEYBOARD)
    return EDIT_VALUE
...
async def edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if field == "due_date":
        ...
        if parsed is False:
            await update.message.reply_text("Data inválida.", reply_markup=_menu(data["kind"]))
            return ConversationHandler.END
        kwargs[field] = parsed
    elif field == "due_time":
        parsed = _parse_time(value)
        if parsed is False:
            await update.message.reply_text("Horário inválido.", reply_markup=_menu(data["kind"]))
            return ConversationHandler.END
        kwargs[field] = parsed
    elif field == "reminder_minutes":
        if not value.isdigit():
            await update.message.reply_text("Informe minutos.", reply_markup=_menu(data["kind"]))
            return ConversationHandler.END
        kwargs[field] = int(value)
    elif field == "details":
        kwargs[field] = None if value == "-" else value
    else:
        kwargs[field] = value
...
async def delete_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup([["✅ Sim", "❌ Não"], ["❌ Cancelar ação"]], resize_keyboard=True),
    )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    action = context.user_data.pop("daily_action", None)
    kind = (data or action or {}).get("kind")
    await update.message.reply_text(
        "Tudo bem, ação cancelada.",
        reply_markup=_menu(kind) if kind else MAIN_KEYBOARD,
    )
    return ConversationHandler.END
...
def _fallbacks():
    return [
        ...
        MessageHandler(filters.Regex(r"^❌ Cancelar ação$"), cancel),
    ]
...
def register_lifestyle_handlers(application) -> None:
    add_conv = ConversationHandler(
        ...
        states={
            ITEM_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_title)],
            ITEM_DATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_date)],
            ITEM_TIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_time)],
            ITEM_REMINDER: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_reminder)],
            ITEM_DETAILS: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_details)],
        },
        fallbacks=_fallbacks(),
    )
    complete_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^☑️ (Concluir tarefa|Concluir compromisso|Resolver pendência)$"), complete_start)],
        states={COMPLETE_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, complete_id)]},
        fallbacks=_fallbacks(),
    )
    edit_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^✏️ (Editar tarefa|Editar compromisso|Editar pendência)$"), edit_start)],
        states={
    ...
    delete_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^🗑️ (Remover tarefa|Remover compromisso|Remover pendência)$"), delete_start)],
        states={
    ...
    application.add_handler(add_conv, group=-1)
    application.add_handler(complete_conv, group=-1)
    application.add_handler(edit_conv, group=-1)
    application.add_handler(delete_conv, group=-1)
