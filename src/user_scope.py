...
def _register_chat(chat_id: int) -> None:
    ...
    with sqlite3.connect(path) as conn:
        conn.execute("""CREATE TABLE IF NOT EXISTS registered_chats (chat_id INTEGER PRIMARY KEY, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)""")
...
def registered_chat_ids() -> list[int]:
    ...
    with sqlite3.connect(path) as conn:
        try: rows = conn.execute("SELECT chat_id FROM registered_chats WHERE active = 1 ORDER BY chat_id").fetchall()
...
def initialize_current_user_storage() -> None:
    ...
    init_database(); init_daily_store(); init_home_tables(); init_assistant_state(); init_finance_tables(); init_natural_tables()
    _INITIALIZED.add(chat_id)


async def establish_user_scope(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat: return
    chat_id = int(update.effective_chat.id); set_current_chat_id(chat_id)
    if multiuser_enabled():
        _register_chat(chat_id); initialize_current_user_storage()
